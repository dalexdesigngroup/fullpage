...
class ET_Builder_Section extends ET_Builder_Structure_Element {
	...
	public function get_fields() {
		$fields = array(
			...
			'fullpage_height'              => array(
				...
				'default'          => 'off',
				'description'      => esc_html__( 'Here you can select whether or not your page should have Fullpage enabled. This must be enabled on all sections to function', 'et_builder' ),
				'tab_slug'         => 'advanced',
				'toggle_slug'      => 'layout',
				'default_on_front' => 'off',
			),
			...
			'inner_shadow'               => array(
				'label'            => esc_html__( 'Enable Horizontal Scroll', 'et_builder' ),
				'type'             => 'yes_no_button',
				'option_category'  => 'configuration',
				'options'          => array(
					'off' => et_builder_i18n( 'No' ),
				...
				'default'          => 'off',
				'description'      => esc_html__( 'Here you can select whether or not your page should have horizontal scrolling enabled. This must be enabled on all sections to function', 'et_builder' ),
				'tab_slug'         => 'advanced',
				'toggle_slug'      => 'layout',
				'default_on_front' => 'off',
			),
		...
		foreach ( $preview_modes as $suffix ) {
			...
			if ( '' !== $background_image && 'on' === $parallax ) {
				...
				if ( 'off' === $parallax_method ) {
					...
					if ( 'off' !== $inner_shadow ) {
						$parallax_classname[] = 'fullpage-slide';
					}
				}
	...
	public function render( $atts, $content, $function_name ) {
		...
		if ( 'on' === $inner_shadow && ! ( '' !== $background_image && 'on' === $parallax && 'off' === $parallax_method ) ) {
			$this->add_classname( 'fullpage-slide' );
		}
...
class ET_Builder_Column extends ET_Builder_Structure_Element {
	...
	public function get_fields() {
		$fields = array(
			...
			'module_fullpageSlide' => array(
				'label'           => esc_html__( 'Enable Dark Theme', 'nemo-new-module' ),
				'type'            => 'yes_no_button',
				'options'         => array(
					'OFF'  => esc_html('No','nemo-new-module'),
					'ON'  => esc_html('Yes','nemo-new-module'),
				),
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'This is a textbox now', 'nemo-new-module' ),
				'tab_slug'        => 'custom_css',
				'toggle_slug'     => 'classes',
				'option_class'    => 'fullpage-slide',
			),
			'admin_label'                 => array(
